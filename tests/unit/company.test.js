...
const BMW_ANAF_RECORD = {
  cui: 49775344,
  name: 'BMW TECHWORKS ROMANIA S.R.L.',
  address: 'JUDEŢUL CLUJ, MUNICIPIUL CLUJ-NAPOCA, STRADA ŞTEFAN CEL MARE, NR.28',
  caenCode: '2620',
  inactive: false,
  inactiveSince: null,
  ...
  administrators: [{ name: 'ADMINISTRATOR BMW', role: 'administrator' }],
  authorizedCaenCodes: ['2620', '6201', '6202', '6209', '7022']
};
...
describe('company.js', () => {
  ...
  describe('getCompanyBrand', () => {
    it('should return the company brand', () => {
      ...
      expect(brand).toBe('BMW TechWorks Romania');
    });
  });

  describe('getCompanyData (no cache)', () => {
    it('should find BMW via ANAF search and return company data', async () => {
      mockFetch
        .mockResolvedValueOnce(anafSearchResponse([
          { cui: 49775344, name: 'BMW TECHWORKS ROMANIA S.R.L.', statusLabel: 'Funcțiune' }
        ]))
        .mockResolvedValueOnce(anafCompanyResponse(BMW_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'BMW TECHWORKS ROMANIA S.R.L.');
      expect(result).toHaveProperty('cif', '49775344');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('BMW TECHWORKS ROMANIA S.R.L.');
    });
    ...
    it('should pick first active company when no exact match', async () => {
      ...
      expect(result.cif).toBe('49775344');
    ...
    it('should throw when no active company found', async () => {
      mockFetch.mockResolvedValueOnce(anafSearchResponse([
        { cui: 11111111, name: 'BMW SOMETHING SRL', statusLabel: 'Radiată' }
      ]));
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(anafCompanyResponse({ cui: 49775344, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.company).toBe('BMW TECHWORKS ROMANIA S.R.L.');
      expect(result.cif).toBe('49775344');
      expect(result.active).toBe(true);
  ...
  describe('validateAndGetCompany', () => {
    it('should return company data with status active', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(peviitorResponse([{ company: 'BMW TECHWORKS ROMANIA S.R.L.' }]));
    ...
    it('should return inactive status when company is inactive', async () => {
      const inactiveRecord = { ...BMW_ANAF_RECORD, inactive: true };
